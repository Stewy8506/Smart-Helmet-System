...
int32_t mic_rx_buffer[MIC_DMA_BUFFER_SIZE];
int16_t amp_tx_buffer[AMP_DMA_BUFFER_SIZE];
...
void AudioPipeline_Start(void)
{
    ...
    HAL_I2S_Receive_DMA(&hi2s3, (uint16_t*)mic_rx_buffer, MIC_DMA_BUFFER_SIZE);
    HAL_I2S_Transmit_DMA(&hi2s2, (uint16_t*)amp_tx_buffer, AMP_DMA_BUFFER_SIZE);
}
...
void HAL_I2S_RxCpltCallback(I2S_HandleTypeDef *hi2s) {
    if (hi2s->Instance == MIC_I2S) {
        ProcessMicData(AUDIO_CHUNK_SIZE * 2); // Start of second half
    }
}

static void ProcessAmpData(int startIndex) {
    if (RingBuffer_GetCount(&Amp_RingBuffer) >= AUDIO_CHUNK_SIZE * 2) {
        RingBuffer_Read(&Amp_RingBuffer, &amp_tx_buffer[startIndex], AUDIO_CHUNK_SIZE * 2);
    } else {
        // Underflow: Output silence
        memset(&amp_tx_buffer[startIndex], 0, AUDIO_CHUNK_SIZE * 2 * sizeof(int16_t));
    }
}
...
{
    if (hi2s->Instance == AMP_I2S) {
        ProcessAmpData(AUDIO_CHUNK_SIZE * 2); // Start of second half
    }
}
